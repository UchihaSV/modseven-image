...
abstract class Image {
     ...
     * @param Image|null $driver driver type: GD, ImageMagick, etc
     * @return  Image
     * @throws Exception
     * @uses    Image::$default_driver
     */
	public static function factory(string $file, self $driver = NULL) : self
	{
		if ($driver === NULL)
		{
			// Use the driver from configuration file or default one
			$configured_driver = Config::instance()->load('image.default_driver');
			$driver = $configured_driver;
		}

		// Set the class name
		//$class = '\\Modseven\\Image\\'.$driver;
	...
	public function __toString()
	{
		...
		catch (Exception $e)
		{
			...
			Log::instance()->log(Log::ERROR, $e->getMessage());
     ...
     */
	public function resize(int $width = NULL, int $height = NULL, int $master = NULL) : self
	{
		if ($master === NULL)
		{
	 ...
	 * @uses    Image::_do_crop
	 */
	public function crop(int $width, int $height, $offset_x = NULL, $offset_y = NULL) : self
	{
		if ($width > $this->width)
		{
			// Use the current width
	 ...
	 * @uses    Image::_do_flip
	 */
	public function flip(int $direction) : self
	{
     ...
     */
	public function reflection(int $height = NULL, int $opacity = 100, bool $fade_in = FALSE) : self
	{
     ...
     */
	public function watermark(Image $watermark, int $offset_x = NULL, int $offset_y = NULL, int $opacity = 100) : self
	{
		if ($offset_x === NULL)
		{
			// Center the X offset
	 ...
	 */
	public function background(string $color, $opacity = 100) : self
	{
		if ($color[0] === '#')
		{
			// Remove the pound
